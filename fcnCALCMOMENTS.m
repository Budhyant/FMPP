function [ momentROTORTHRUST, momentROTORPx, momentROTORPy, momentROTORMx, momentROTORMy, momentROTORQ,...
            ...
            rotorTHRUST, rotorPx, rotorPy, rotorMx, rotorMy, rotorQ,pitchVEHICLEdeg, geomNumROTORS)
  ...
  % preallocate for number of rotors
    ...
    for i = 1:geomNumROTORS
        ...
        if i == 1 || i == 3 %
            ...
            momentROTORQ(:,1:3,i) = [0,0,rotorQ(i)];
        else
            rotorPy2(i) = -rotorPy(i);
...
% Calculate total moments - Q (torque) added Nov 27 2017
momentTOTAL = sum(momentROTORTHRUST(end,:,:),3) + sum(momentROTORPx(end,:,:),3) + ...
    sum(momentROTORPy(end,:,:),3) + sum(momentROTORMx(end,:,:),3) + ...
    sum(momentROTORMy(end,:,:),3) +sum(momentROTORQ(end,:,:),3)+ ...
    sum(momentWEIGHTMOTOR(end,:,:),3) + sum(momentWEIGHTARM(end,:,:),3) + ...
    sum(momentDRAGMOTOR(end,:,:),3) + sum(momentDRAGARM(end,:,:),3)+ ...
    sum(momentWEIGHTLEG(end,:,:),3) + sum(momentDRAGLEG(end,:,:),3) + ...
    momentWEIGHTBODY(end,:) + momentWEIGHTPAYLOAD(end,:) + ...
    momentDRAGBODY(end,:) + momentDRAGPAYLOAD(end,:) + ...
    momentLIFTBODY(end,:) + momentDRAGBODYinduced(end,:) +...
    momentWEIGHTOFFSET(end,:);
                                                                %%%%%%%%%%%%%%
