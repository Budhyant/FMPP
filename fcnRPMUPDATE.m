function [vecANG, magvecVR, vecVR, vecINT, vecBODY, rotorRPM, ...
    ...
    rotorMUinf] ...
     = fcnRPMUPDATE(flowq, flowRHO, Vint, geomNumROTORS, rotorTHRUST,...
 ...
 % lookup new rotor performance data based on new inflow velocity and angle         
         for i = 1:geomNumROTORS
            [ rotorRPM(:,:,i), rotorPx(:,:,i), rotorPy(:,:,i), ...
                rotorMx(:,:,i), rotorMy(:,:,i), rotorQ(:,:,i),...
                rotorCP(:,:,i), rotorMUinf(:,:,i) ] ...
                    = fcnRPMLOOKUP( q(:,:,i), flowRHO, vecANG(:,:,i), rotorTHRUST(:,:,i), ...
                    tabLOOKUP, vecANGLST, geomDIAMETER );
                
%             [ rotorRPM(:,:,i), rotorPx(:,:,i), rotorPy(:,:,i), ...
%                 rotorMx(:,:,i), rotorMy(:,:,i), rotorQ(:,:,i),...
%                 rotorCP(:,:,i), rotorMUinf(:,:,i) ] ...
%                     = fcnRPMLOOKUPv2_new( q(:,:,i), flowRHO, vecANG(:,:,i), rotorTHRUST(:,:,i), ...
%                     tabLOOKUP, vecANGLST );    
