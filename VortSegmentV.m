...
tempwi = tempwi.*[idx idx idx]; %rows include induced velocities for all segments and rings for each rotor

wi = zeros(size(pointInterest,1),3,size(matSEG1,3));

% separate tempwi rows into different matrices and sum for induced velocity
% on each point of interest (i.e. number of rotors)
for i = 1:size(pointInterest,1)
